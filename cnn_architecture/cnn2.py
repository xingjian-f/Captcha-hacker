...
def build_cnn(img_channels, img_width, img_height, max_nb_cha, nb_classes):
    ...
    loss = {'output%d'%i:log_prob for i in range(1, max_nb_cha+1)}
    loss['output_nb'] = log_prob
    metrics={'output%d'%i:'categorical_accuracy' for i in range(1, max_nb_cha+1)}
    metrics['output_nb'] = 'categorical_accuracy'
